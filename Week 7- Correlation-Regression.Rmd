...
Positive correlations mean that as one variable increases, the other also increases.
]
...
ggplot(crime[1:30, ], aes(x = E, y = (FoC - predict(foc_by_E)[1:30]) ^2)) + 
  geom_point(alpha = 0.5)+
  geom_hline(aes(yintercept = 0)) +
  geom_segment(aes(xend = E, yend = 0), linetype = "dashed") + 
  theme_classic(base_size = 20) + 
  labs(y = "Difference^2 from prediction",
       x = "Emotionality")
```
]
